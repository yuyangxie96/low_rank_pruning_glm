...
def get_lowrank_tuple(tensor, max_rank):
	...
	_key_id = cache_path + str(np.round(np.abs(float(tensor.sum())),8)) + '_' + str(max_rank) + '_' + str(np.abs(tensor[0][0].numpy())) + '.npy'
	try:
...
# 		weight_A, weight_B  = get_lowrank_tuple(weight_cpu, max_rank=max_rank) 
# 		weight_A, weight_B  = (weight_A.half(), weight_B.half()) if(fp16) else (weight_A.float(), weight_B.float())

# 		self.weight_A = torch.nn.Parameter(weight_A.contiguous(), requires_grad=True)
# 		self.weight_B = torch.nn.Parameter(weight_B.contiguous(), requires_grad=True)
...
def linear_lowrank(linear_layer, patch_params, device='cuda', return_layer=True):
	...
	linear_layer.weight_A = torch.nn.Parameter(weight_A.to(device), requires_grad=True).contiguous()
	linear_layer.weight_B = torch.nn.Parameter(weight_B.to(device), requires_grad=True).contiguous()
	...
	#########################
	def forward_AB(x):
		out = torch.matmul(torch.matmul(x, linear_layer.weight_A), linear_layer.weight_B)
...
def patch_linear_lowrank_no_peft(linear_layer, patch_params, device='cuda'):
	...
	#new_layer = linear_lowrank(linear_layer, patch_params, device=device)
	return new_layer
